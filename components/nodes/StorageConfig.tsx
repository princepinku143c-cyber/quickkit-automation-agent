...
const StorageConfig: React.FC<StorageConfigProps> = ({ subtype, config, onChange }) => {
    ...
    // --- 6. MONGODB - THE NOSQL CHAMPION ---
    if (subtype === NexusSubtype.MONGODB) {
        return (
            <div className="space-y-2">
                {/* TIER 1: QUICK OPERATION */}
                <CollapsibleSection icon={Database} title="Quick Operation" defaultOpen={true}>
                    <SelectField 
                        ...
                        options={[
                            ...
                            {label: 'Aggregate Pipeline', value: 'AGGREGATE'},
                            {label: 'Count Documents', value: 'COUNT'}
                        ]} 
                    />
                    <div className="mt-3">
                        <InputField label="Collection" value={config.collection} onChange={(v: string) => onChange('collection', v)} placeholder="users" />
                    </div>
                    {['FIND', 'FIND_ONE', 'COUNT', 'DELETE_ONE', 'DELETE_MANY'].includes(config.operation) && (
                        <div className="mt-3">
                            <TextAreaField label="Filter / Query (JSON)" value={config.query} onChange={(v: string) => onChange('query', v)} rows={3} placeholder='{ "status": "active" }' />
                        </div>
                    )}
                </CollapsibleSection>

                {/* TIER 2: CONNECTION (ADVANCED) */}
                <CollapsibleSection icon={Server} title="Connection String">
                    <InputField label="Connection URI" value={config.uri} onChange={(v: string) => onChange('uri', v)} placeholder="mongodb+srv://user:pass@cluster.mongodb.net/db" />
                    <div className="mt-3 p-3 bg-nexus-900/50 rounded border border-nexus-800">
                        <ToggleField label="Enable TLS/SSL" value={config.tls} onChange={(v: boolean) => onChange('tls', v)} />
                        {config.tls && (
                            <div className="mt-2">
                                <ToggleField label="Allow Invalid Certs" value={config.allowInvalidCerts} onChange={(v: boolean) => onChange('allowInvalidCerts', v)} activeColor="bg-yellow-500" />
                            </div>
                        )}
                    </div>
                </CollapsibleSection>
                ...
                {(config.operation === 'INSERT_ONE' || config.operation === 'INSERT_MANY' || config.operation === 'UPDATE_ONE' || config.operation === 'UPDATE_MANY') && (
                    <CollapsibleSection icon={Edit} title="Write Operations">
                        <TextAreaField 
                            label={config.operation.includes('MANY') ? "Documents Array (JSON)" : "Document (JSON)"}
                            value={config.document} 
                            onChange={(v: string) => onChange('document', v)} 
                            rows={5} 
                        ...
                        {config.operation.includes('UPDATE') && (
                            <>
                                <div className="mt-3">
                                    <TextAreaField label="Update Filter (Where)" value={config.query} onChange={(v: string) => onChange('query', v)} rows={2} placeholder='{ "email": "john@example.com" }' />
                                </div>
                                <div className="mt-3">
                                    <ToggleField label="Upsert (Insert if not found)" value={config.upsert} onChange={(v: boolean) => onChange('upsert', v)} />
                                </div>
                            </>
                        )}
                    </CollapsibleSection>
                ...
                {config.operation === 'AGGREGATE' && (
                    <CollapsibleSection icon={Layers} title="Aggregation Pipeline">
                        <TextAreaField label="Pipeline Stages (JSON Array)" value={config.pipeline} onChange={(v: string) => onChange('pipeline', v)} rows={8} placeholder='[
  { "$match": { "status": "active" } },
  { "$group": { "_id": "$category", "total": { "$sum": "$amount" } } }
]' />
                        <p className="text-[9px] text-gray-500 mt-2">Use standard MongoDB aggregation stages.</p>
                    </CollapsibleSection>
                )}

                {/* TIER 5: INDEXES & PERFORMANCE */}
                <CollapsibleSection icon={BarChart3} title="Indexes & Options">
                    {['FIND', 'FIND_ONE'].includes(config.operation) && (
                        <>
                            <TextAreaField label="Projection (Select Fields)" value={config.projection} onChange={(v: string) => onChange('projection', v)} rows={2} placeholder='{ "name": 1, "email": 1, "_id": 0 }' />
                            <div className="grid grid-cols-2 gap-4 mt-3">
                                <TextAreaField label="Sort (JSON)" value={config.sort} onChange={(v: string) => onChange('sort', v)} rows={1} placeholder='{ "created_at": -1 }' />
                                <InputField label="Limit" type="number" value={config.limit} onChange={(v: string) => onChange('limit', parseInt(v))} placeholder="100" />
                            </div>
                            <InputField label="Skip" type="number" value={config.skip} onChange={(v: string) => onChange('skip', parseInt(v))} placeholder="0" className="mt-2" />
                        </>
                    )}
                    
                    <div className="mt-3 border-t border-nexus-800/50 pt-3">
                        <SelectField label="Read Preference" value={config.readPreference || 'primary'} onChange={(v: string) => onChange('readPreference', v)} options={[{label: 'Primary', value: 'primary'}, {label: 'Secondary', value: 'secondary'}, {label: 'Nearest', value: 'nearest'}]} />
                    </div>
                </CollapsibleSection>

                {/* TIER 6: ADVANCED FEATURES */}
                <CollapsibleSection icon={Zap} title="Advanced Features" badge="Pro">
                    <ToggleField label="Use Transaction" value={config.transaction} onChange={(v: boolean) => onChange('transaction', v)} description="Requires Replica Set." />
                    <ToggleField label="Change Stream (Watch)" value={config.changeStream} onChange={(v: boolean) => onChange('changeStream', v)} description="Trigger on database changes." />
                </CollapsibleSection>
            </div>
        );
    }
